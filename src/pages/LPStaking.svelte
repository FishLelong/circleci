...
<div class="content flex flex-col">
    ...
    <div class="liquidity-container flex flex-col align-center bg-grey-243 rounded-4px p-4 my-0 md:p-6 w-full">    
        {#if !pool}
            <h1 class="text-center text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl ">{ $_('stake.select.a.pool') }</h1>
                ...
                {#each filter(incentivizedPools, {deprecated: false}) as ammPool}
                    {#if ammPool.highlight}
                        <div class="highlight-box farming-card flex flex-col justify-center align-center items-center text-center mx-2 my-2 md:m-2 border border-gray border-opacity-50 border-solid rounded-sm p-6">
                            ...
                            {#if ammPool.enabled}
                              <button on:click={() => pool = ammPool } class="btn border-white clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4">{$_('general.select')}</button>
                            {:else}
                        ...
                        <img class="h-40px w-40px mb-2 md:h-70px md:w-70px"src={images.logos.piedao_clean} alt="PieDAO logo" />
                          ...
                          {#if ammPool.contractType === 'Geyser'}
                            <div style="position:absolute; top:10px; right:10px;" class="apy">
                              <!-- <a target="_blank" href="https://forum.piedao.org/t/pip-20-week-2-incentive-programs/197">ℹ️</a> -->
                            </div>
                          ...
                          {#if ammPool.enabled}
                            <input type="hidden" value={pool}/>
                            <button on:click={() => pool = ammPool } class="btn clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4">{$_('general.select')}</button>
                          {:else}
                            <button disabled class="btn border-white clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4">Oct 3rd, 6:00pm UTC</button>
                          {/if}
                      </div>
            ...
            <!-- <h1 class="mt-8 mb-1 px-2 text-center text-lg md:text-xl">⚠️ { $_('stake.deprecated.pools') }</h1>
            <div class="flex flex-col w-full justify-center md:flex-row">
        ...
        {:else}
       <div>
              <button on:click={() => pool = null } class="md:w-1 float-left btn clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4">{$_('general.back')}</button>
            ...
            <div class="flex flex-col w-full justify-around md:flex-row">
              ...
              <div class="farming-card flex flex-col justify-center align-center items-center mx-1 my-4  border border-gray border-opacity-50 border-solid rounded-sm py-2">
                    ...
                    <div class="w-80 input bg-white border border-solid rounded-8px border-grey-204 mx-0 md:mx-4">
                        <div class="top h-24px text-sm font-thin px-4 py-4 md:py-2">
                            <div class="left float-left">Amount to unstake</div>
                        </div>
                    ...
                    {#if amountToUnstake === 0 }
                        <button disabled class="btn clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4 border-white">{$_('stake.enter.amount')}</button>
                    {:else}
                      <button on:click={() => unstake()} class="btn clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4">{$_('general.unstake')}</button>
                    {/if}
              ...
              <div class="farming-card highlight-box flex flex-col justify-center align-center items-center mx-1 my-4  border border-grey border-opacity-50 border-solid rounded-sm py-2">
                    ...
                    <div class="w-80 input bg-white border border-solid rounded-8px border-grey-204 mx-0 md:mx-4">
                        <div class="top h-24px text-sm font-thin px-4 py-4 md:py-2">
                            <div class="text-black left black float-left">Amount to stake</div>
                        </div>
                        <div class="bottom px-4 py-4 md:py-2">
                    ...
                    {#if needAllowance }
                      <button on:click={ () => action(pool, 'unlock')} class="btn clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4 border-white">{$_('stake.approve')}</button>
                    {:else}
                      {#if amountToStake === 0 }
                        <button disabled class="btn clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4 border-white">{$_('stake.enter.amount')}</button>
                      {:else}
                        <button on:click={() => stake()} class="btn clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4 border-white">{$_('general.stake')}</button>
                      {/if}
                    {/if}
              </div>
              ...
              <div class="farming-card flex flex-col justify-center align-center items-center mx-1 my-4  border border-gray border-opacity-50 border-solid rounded-sm py-2">
                    ...
                    <div class="w-80 input bg-white border border-solid rounded-8px border-grey-204 mx-0 md:mx-4">
                        <div class="top h-24px text-sm font-thin px-4 py-4 md:py-2">
                            <div class="left float-left">Amount to claim</div>
                        </div>
                        <div class="bottom px-4 py-4 md:py-2">
                    ...
                    <button on:click={() => getRewards()} class="btn clear font-bold ml-1 mr-0 rounded md:mr-4 py-2 px-4">{$_('stake.claim')}</button>
              </div>
              {/if}
            ...
            <div class="info-box">
              {#if $farming[pool.addressUniPoll] !== undefined}
                <p>{$_('stake.total.of')}  : <strong>{toFixed($farming[pool.addressUniPoll].totalBPTAmount, 4)} BPT </strong>.</p>
                <p>{$_('stake.total')}    : <strong>{toFixed($farming[pool.addressUniPoll].totalStakedBPTAmount, 4)} BPT</strong> {$_('stake.staked.contract')}</p>
              ...
              {#if pool.contractType === "Geyser"}
                <br/>
                <p> Your stake of <strong>{geyserApy.yourStake.toFixed(2)} BPT</strong> is earning right now <strong>{geyserApy.earnedOptimistic} CEPToken</strong> assuming you will not unstake until the end of the program. </p>
                <p>Approx <strong>{geyserApy.apy}%</strong>.</p>
                <p>If you would unstake right now, you would get exactly <strong>{geyserApy.earnedNotOptimistic} CEPToken</strong>, which is approx <strong>{geyserApy.apyNotOptimistic}% APR</strong>.</p>
              {/if}

              <br/><br/>
              <p>{$_('stake.add.liquidity')} {pool.platform} {$_('general.pool')} {$_('stake.to.get')} {pool.toStakeSymbol} {$_('stake.tokens')} <a href={pool.poolLink}>{$_('stake.here')}</a></p>
              <p>{$_('stake.weekly.rewards')} <strong>{pool.weeklyRewards} {pool.rewards_token}</strong></p>
              <p>{$_('general.buy')} {pool.containing[0].symbol} {$_('general.on')} <a target="_blank" href="https://balancer.exchange/#/swap/ether/{pool.containing[0].address}">Balancer</a> {$_('general.or')} <a href="#/pools/{pool.containing[0].address}">mint now!</a></p>
              <p>{$_('general.buy')} CEPToken {$_('general.on')} <a target="_blank" href="https://balancer.exchange/#/swap/ether/0xad32A8e6220741182940c5aBF610bDE99E737b2D">Balancer</a></p>
            </div>
        {/if}
    </div>
</div>
