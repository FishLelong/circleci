  ...
  <div class="content flex flex-col spl">
      ...
      <div class="flex flex-row content-between justify-between flex-wrap w-full">
        ...
        <div class="sm:w-full md:w-2/3 flex flex-row-reverse">
          <a href={`#/pools/${token}/withdraw/multi`}>
            <button class="btn text-white font-bold ml-0 mr-1 rounded md:ml-4 py-2 px-4">{$_('cep.redeem')}</button>
          </a>
          <a href={`#/pools/${token}`}>
            <button class="btn text-white font-bold ml-0 mr-1 rounded md:ml-4 py-2 px-4">{$_('cep.mint')}</button>
          </a>
    ...
    <div class="flex justify-between flex-wrap w-full mt-2 md:mt-8">
      ...
      <div class="p-0 self-start md:w-1/4">
        <div class="text-center font-thin text-xs md:text-base">{$_('cep.marketCap')}</div>
      ...
      <div class="p-0 md:w-1/4">
        <div class="text-center font-thin text-xs md:text-base">{$_('cep.swap.fee')}</div>
        <div class="text-center text-2xl md:text-xl font-black">{swapFees}%</div>
      </div>
      <div class="p-0 md:w-1/4">
        <div class="text-center font-thin text-xs md:text-base">{$_('cep.streaming.fee')}</div>
        <div class="text-center text-2xl md:text-xl font-black">0%</div>
      </div>
      <div class="p-0 md:w-1/4">
        <div class="text-center font-thin text-xs md:text-base">{$_('cep.days.change')}</div>
        <div class="text-center text-2xl md:text-xl font-black">
    ...
    <h1 class="mt-8 mb-4 text-base md:text-3xl">{$_('cep.allocation.breakdown')}</h1>
    {#if pieOfPies }
      <h4>*This allocation is composed of multiple pies, find below the exploded allocation.</h4>
      <ul>
        {#each pieOfPies as subPie}
          <li><a href="#/cep/{subPie.address}">{subPie.symbol}</a></li>
        {/each}
      </ul>
    {/if}
    ...
    <div class="w-99pc m-4">
      <table class="breakdown-table table-auto w-full">
        <thead>
          <tr>
            <th class="font-thin border-b-2 px-4 py-2 text-left">{$_('cep.asset.name')}</th>
            <th class="font-thin border-b-2 px-4 py-2">{$_('cep.price')}</th>
            <th class="font-thin border-b-2 px-4 py-2">{$_('cep.current.allocation')}</th>
            <th class="font-thin border-b-2 px-4 py-2">{$_('cep.marketCap')}</th>
            <th class="font-thin border-b-2 px-4 py-2">{$_('cep.volume')}</th>
            <th class="font-thin border-b-2 px-4 py-2">{$_('cep.change')}</th>
          </tr>
        </thead>
        <tbody>
    ...
    <!-- <h1 class="mt-8 mb-4 text-base md:text-3xl">{$_('cep.latest.news')}</h1>
  
    <div class="flex flex-col w-full justify-between md:flex-row">
      <div class="w-full md:w-49pc">
        <blockquote class="twitter-tweet"><p lang="en" dir="ltr">Membership Proposal ✅ passed<br><br>The proposal will enable members <a href="https://twitter.com/TheLAOOfficial?ref_src=twsrc%5Etfw">@TheLAOOfficial</a> to join <a href="https://twitter.com/CEP_project_">@CEProject</a>, a fee-collecting organization coordinating weights of “tokenized asset allocations”, PIEs that is open to anyone <a href="https://twitter.com/ethereum?ref_src=twsrc%5Etfw">@ethereum</a> through the purchase of its voting shares, <a href="https://twitter.com/CEP_project_">$CEPToken</a> 💜 <a href="https://t.co/ikCkLTeGJy">https://t.co/ikCkLTeGJy</a> <a href="https://t.co/4L13l1xsXl">pic.twitter.com/4L13l1xsXl</a></p>&mdash; ⟠ DeFi++ (@defiopfi) <a href="https://twitter.com/defiopfi/status/1299419505018515457?ref_src=twsrc%5Etfw">August 28, 2020</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
      </div>
      <div class="w-full md:w-49pc">
